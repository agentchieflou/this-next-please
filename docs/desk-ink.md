...
| `underline` | a line under the text, a little past both ends: 2px under the tallest box on its text's line (the anchor's siblings whose height overlaps it, so a line past its end passes under a chip beside a name), and never lower than 2px over the next row's top, the first element after it in its pane that starts below it (#331) | `text-decoration: underline` |
