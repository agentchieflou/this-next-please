...
| napkin skin (#252) | the quilted stock under the page; per pane a coffee ring, the felt tip's bleed along its error box, and the running pen's tip; the paper state grammar as a mark table | `static/ink/skins/napkin.js` (`paper`, `frame`, `tick`), reading `state-*`, `is-done`, `needs-human`, `.chip.stale`, `.oldsession`, `.ask-q`, `.ask-choice[aria-pressed]`, `li.denied`/`li.friction` and `#bellcount` -- never writing them | `static/skins/napkin/skin.css` (`--paper`, `--paper-seam`, `--coffee`, `--ink-*`); panes transparent under ink | `diner`, `kraft`; the ring under `.state-idle` with a `.stale` chip; under `body.ink-off` the quilt and ring in CSS and the marks plain | — | `test_fleet_napkin.py` |
