...
| activity trace | sixty bars, one a minute; the same hour as `data-trace` (`peak\|n n n!`, #253) | `drawTrace` | `.trace` | red where a minute needed a person; transparent (still its sentence) while the graph paper plots it | — | `test_fleet_trace.py`, `test_fleet_ink_graph.py` |
