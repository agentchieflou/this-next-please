...
| activity trace | sixty bars, one a minute; the same hour as `data-trace` (the peak, then a count a minute, #253) | `drawTrace` | `.trace` | red where a minute needed a person; transparent (still its sentence) while the graph paper plots it | — | `test_fleet_trace.py`, `test_fleet_ink_graph.py` |
...
| napkin skin (#252) | the quilted stock under the page; per pane a coffee ring and the felt tip's bleed along its error box; the paper state grammar as the notebook's mark table | `static/ink/skins/napkin.js` (`paper`, `frame`, `tick`), reading `state-*`, `is-done`, `needs-human`, `.chip.stale`, `.oldsession`, the question card (`is-answered`, `aria-pressed`), `li.friction` and `#bellcount` -- never writing them | `static/skins/napkin/skin.css` (`--paper`, `--paper-seam`, `--coffee`, `--ink-*`); panes transparent under ink | `diner`, `kraft`; the ring under `.state-idle` with a `.stale` chip; under `body.ink-off` the quilt and ring in CSS and the marks plain | — | `test_fleet_napkin.py` |
