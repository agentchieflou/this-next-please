...
| farmstead on three.js | the soil, the header and footer planks, and per pane a shadow, its paper, a lit wooden frame and the crop in the chip's glyph box | `static/ink/skins/farmstead.js` through the ink layer's hooks, from the tile's `state-*` and `needs-human`; never writes the page | `static/skins/farmstead/skin.css` (`--farm-*` inputs; clears the header, footer, pane and chip backgrounds while ink is on) | the crop per state (seed, sprout, sun, bloom, wilted), grown a stage per advance; a scorched frame on `state-error`; the plain farm under `body.ink-off` (#255) | — | `test_fleet_ink_farmstead.py` |
