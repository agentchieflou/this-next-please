...
| activity trace | the hour, a point a minute: the ink layer's pen line where a skin draws with ink (#257), else its own SVG | `drawTrace` (writes `data-ink-series`, and the counts as `data-trace` for a skin that plots them, #253) | `.trace` | a red tick where a minute needed a person | — | `test_fleet_trace.py` |
| the ground | three blobs: glass's own `ground` hook in the ink layer's `ground` slot where the shell draws ink, else the stylesheet's gradients, still (#257) | `static/ink/skins/glass.js`; no script without ink | `body` | still under reduced motion | — | `test_fleet_trace.py` |
...
| ink layer | one canvas behind the page; a lane per pane and one for the header; the marks a skin's table derives from the page's classes | `static/ink/` (`Ink.setSkin`), never `app.js`; its only page write is a handwriting reveal's `clip-path`, while it is written | `#ink` | `body.ink-off` (the plain fallback, a constructed stylesheet); drawn at once under reduced motion; glass (#254), the graph paper (#253) and the notebook (#249, #250) draw with it | — | `test_fleet_ink.py`, `test_fleet_probe.py` |
| graph paper skin (#253) | the paper and its 28px grid, the state grammar's marks, each pane's hour plotted on the grid | `static/ink/skins/graph.js` (a skin module: marks, `paper`, `frame`, `tick`), never `app.js` | `static/skins/graph/skin.css` | `engineering`, `blueprint`; `body.ink-off` draws the same table plain on a CSS grid | — | `test_fleet_ink_graph.py` |
| voxel slabs (#256) | per pane, one lit slab: its drop shadow, the panel block the text is read on, three sockets and the accent strip as a column of cubes; the ground as 32px voxels | `static/ink/skins/voxel.js` (`ground`, `paper`, `frame`): one instanced mesh per material, placed by a uniform per pane, never `app.js` | `#ink`; `skin.css` stops painting the tile's fill, borders and shadow where the gate is on | the variant's `--voxel-*` surfaces; under `body.ink-off` the CSS skin as before | — | `test_fleet_voxel_ink.py` |
| voxel status stack (#256) | per pane, up to three blocks in the strip's sockets, a crack's other half, a pebble, an ore fleck | `static/ink/skins/voxel.js` (`tick`), read from `state-*`, `needs-human`, `.oldsession`, `.transcript li.friction`/`li.denied` | `#ink`; replaces `.chip::before` where the gate is on | the grammar in `docs/skin-voxel.md`: turned (running), raised (needs you), cracked (error), full (done), pebble (stale), ore (a finding); at rest under reduced motion | — | `test_fleet_voxel_ink.py` |
| farmstead on three.js | the soil, the header and footer planks, and per pane a shadow, its paper, a lit wooden frame and the crop in the chip's glyph box | `static/ink/skins/farmstead.js` through the ink layer's hooks, from the tile's `state-*`, `is-done` and `needs-human`; never writes the page | `static/skins/farmstead/skin.css` (`--farm-*` inputs; clears the header, footer, pane and chip backgrounds while ink is on) | the crop per state (seed, sprout, sun, bloom, wilted), grown a stage per advance; a scorched frame on `state-error`; the plain farm under `body.ink-off` (#255) | — | `test_fleet_ink_farmstead.py` |
| the legal pad | canary stock, blue rules, a double red margin per pane, the gummed band; the paper state grammar as its mark table, the running pen's tail and the struck header count drawn beside it (#251) | `static/ink/skins/legalpad.js` (the ink layer calls it), `skins/legalpad/skin.css`; never `app.js` | `#ink` (drawn), `body[data-skin="legalpad"]` | `body.ink-off`: the same pad in CSS gradients under the plain marks; drawn at once under reduced motion | — | `test_fleet_ink_legalpad.py`, `test_fleet_skins.py` |
| napkin skin (#252) | the quilted stock under the page; per pane a coffee ring, the felt tip's bleed along its error box, and the running pen's tip; the paper state grammar as a mark table | `static/ink/skins/napkin.js` (`paper`, `frame`, `tick`), reading `state-*`, `is-done`, `needs-human`, `.chip.stale`, `.oldsession`, `.ask-q`, `.ask-choice[aria-pressed]` and `#counts` -- never writing them | `static/skins/napkin/skin.css` (`--paper`, `--paper-seam`, `--coffee`, `--ink-*`); the row transparent under ink | `diner`, `kraft`; the ring under `.state-idle` with a `.stale` chip; under `body.ink-off` the plain look and the marks plain | — | `test_fleet_napkin.py` |
...
| model card | facts (configured, its source, the last turn's), the full model picker, the note (`#mc-note`, `role=status`: saved, refused, pinned, effort reset, not offered) | `openModelCard`, `drawModelCard` | `.modelcard` | `aria-expanded` on the `.modeltoggle` that opened it; `.mp-other.bad` after a refusal of what was typed | `m` on a pane or a rail; the picker's arrows, `Home`, `End`, `Enter`; every other key stays in the card; `Esc` closes it and gives the keyboard back | `test_fleet_column.py`, `test_fleet_ink.py` |
| model picker (#362) | the model toolbar (`.mp-models`): a group per provider in the catalogue's order (`.mp-glabel`, then pills: `.pill-mark`, `.pill-label`, `.pill-note`), `other…` last; `input.mp-other`; the full picker's effort toolbar (`.mp-effort`) and `.mp-effort-why`. Compact: the pressed pill, the default, two quick ids, `more…` | `drawModelPicker` (`picker.js`), built once by `createModelPicker`; hosted by /settings (#367) | `.mpick` | `data-variant` `full`, `compact`; pressed (`aria-pressed`, ✓, a 2px ring on `--select`); the last turn's (•); not offered (⚠); unavailable (⊘, dashed, `aria-disabled`, its reason by `aria-describedby`); every effort `aria-disabled` while inheriting no model | one tab stop per toolbar; `←` `→` `↑` `↓`, `Home`, `End`, `Enter`, `Space`; `Enter` in `other…`'s input; `Esc` is the host's | `test_fleet_model_picker.py` |
| settings page | appearance, models, Copilot, permissions | `settings.js` | `body.settings-page` | — | — | `test_fleet_settings_page.py` |
| settings model block (#367) | `every agent`'s full picker, the list's line (`#modellist`) and *refresh the list*, then a row per repository (`#model-<repo>`): its name, a compact picker whose `more…` opens a full one in the same cell, resolved from, last turn actually used | `drawModels` (`settings.js`), the rows keyed by `patchList` | `#fleetpicker`, `.mp-expand` | a row's expansion open (`more…` `aria-expanded`), one at a time | the picker's; `Esc` closes a row's expansion | `test_fleet_settings_page.py` |
