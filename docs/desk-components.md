...
| settings page | appearance, models, Copilot, permissions | `settings.js`; the model block by `drawModels`: the fleet-wide picker, then one row per repository, keyed (`patchList`), with a compact picker whose `more…` opens a full one in the same cell (#367) | `body.settings-page`, `.mp-expand` | a row's expansion open (`more…` `aria-expanded`), one at a time | `Esc` closes a row's expansion | `test_fleet_settings_page.py` |
