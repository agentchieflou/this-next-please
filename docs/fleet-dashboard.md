...
the on-disk figure reported beside it so a file that doubles is still visible. Since #523 (decision
18 on #429) the prose is not in the page at all: a served script or stylesheet carries code only, its
reasoning is in `<file>.md` beside it, which the server never serves, and the desk went from 202,333
bytes gzipped to 117,838 ([desk-components.md](desk-components.md) §Where a component's reasoning
lives). On loopback the
