...
|---|---|---|
| `fleet.model`, `fleet.effort` | the model every agent is launched with, set on /settings by pressing a model; **CLI default** passes no flag and the CLI chooses | from the agent's next turn |
| `fleet.models.<repo>` | `{"model": …, "effort": …}` for one repository, over the fleet-wide default, set on /settings by pressing a model in its row; **inherit** removes it | from the agent's next turn |
