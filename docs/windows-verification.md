...
| P17 | The snap-back (#230), in Chrome: `ad-fleet serve --open`, with three agents on the desk. Click one agent, then another, then a third, a second apart, and wait ten seconds on each | each click stays where it landed. The pane you opened is still the open one ten seconds later, and nothing reopens the one before it | _not yet measured_ | the symptom was *"it automatically snaps back to the one I was previously on"*, within a second of the click. `tests/regressions/test_20260922_any_chrome_snapback.py` holds it headless; this is the same click with the laptop's own `desk.json` |
| P18 | The stale questions (#231), in Chrome: in one agent's console, ask a question with `ad-state ask`, answer it from the desk's card, then ask a second and clear it with `ad-state set --clear-questions` | the tile's count goes 1, 0, 1, 0, and the card leaves each time. `ad-state show` agrees with the tile at every step | _not yet measured_ | the symptom was *"12 questions, but all of them have been answered"*. `tests/regressions/test_20260922_any_answered_questions.py` holds the fold; this is the same round trip through a real console |
