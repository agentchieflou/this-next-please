...
| Fleet | `ad-fleet friction` / `POST /api/friction` with no repo or an unregistered one (#499) | `refused: no_repo`, exit 2, with a hint | `test_fleet_board_desk.py::test_dismissing_friction_writes_the_fleet_directory_and_never_the_checkout` |
| Fleet | `ad-fleet friction --dismiss` / `POST /api/friction` naming a file not in `.agent/friction/` (#499) | `refused: not_friction`, exit 2, hint naming `ad-fleet friction <repo>` | `test_fleet_board_desk.py::test_the_cli_and_the_page_dismiss_through_one_function` |
...
| Git push | HEAD is detached | `refused: detached_head`, exit 2, nothing pushed | `test_git_push.py::test_a_detached_head_is_refused` |
| Git push | `--remote` is not a remote `git remote` lists (a URL included) | `refused: unknown_remote`, exit 2, naming the configured ones | `test_git_push.py::test_a_remote_that_is_not_configured_is_refused` |
| Git push | the current branch is `main`, `master`, `develop` or the remote's HEAD (read locally, then asked of the remote) | `refused: default_branch`, exit 2, quoting AGENTS.md rule 16 | `test_git_push.py::test_a_protected_branch_is_refused` |
...
| Approval gate | `ad-git push` inside a fleet, denied | `refused: approval_denied`, exit 2, nothing pushed | `test_git_push.py::test_inside_a_fleet_a_deny_pushes_nothing` |
| Wrap-up | `--confirm` names a preview that is no longer the fresh one, or the desk's `job` is not the repo's latest | `refused: plan_changed`, exit 2 / 409, the new table; nothing written | `test_fleet_wrapup.py::test_the_cli_previews_refuses_without_a_confirm_and_refuses_a_stale_one` |
| Wrap-up | the PR or page adapter has no pinned verb yet (#506, #507) | the row reads `code: not_pinned`, unticked, naming `ad-pncli capture-help` | `test_fleet_wrapup.py::test_end_of_day_on_a_ticketed_branch_with_three_unpushed_commits` |
| Wrap-up | the agent is mid-turn, in a console, or your own chat | every row `code: busy`, skipped with a hint; nothing runs, nothing is queued | `test_fleet_wrapup.py::test_a_mid_turn_agent_gets_every_row_skipped_and_nothing_runs` |
| Wrap-up | a bare `ad-fleet wrapup`, or no `--confirm` | exit 2, *name a repo* / *confirm with `--confirm <plan_id>`*; nothing written | `test_fleet_wrapup.py::test_the_cli_previews_refuses_without_a_confirm_and_refuses_a_stale_one` |
| Wrap-up | a bare `ad-fleet wrapup`, or `POST /api/wrapup` with neither `repo` nor `all` (#505) | `refused: no_repo`, *name a repo, or pass --all*, exit 2 / 409; nothing runs | `test_fleet_wrapup.py::test_the_sweep_cli_and_api_answer_in_the_same_words_and_codes` |
| Wrap-up | the sweep's `--confirm` names a fleet preview that changed, or the desk's sweep `job` is not the latest (#505) | `refused: plan_changed` (or `confirm_required` without `--confirm`), exit 2 / 409, the new table; nothing written | `test_fleet_wrapup.py::test_the_sweep_cli_and_api_answer_in_the_same_words_and_codes` |
| Wrap-up | an agent is mid-turn during the sweep (DAY-D3) | its repo row `state: busy`, every step `code: busy`, with the hint; never queued | `test_fleet_wrapup.py::test_the_day_sweep_before_506_and_507_land` |
