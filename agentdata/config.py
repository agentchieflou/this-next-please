...
def min_speedup(cfg: dict | None = None, root: str | None = None) -> float:
    """The speedup `test-regress` requires before a change counts as an optimisation (default 1.10).

    Same precedence as min_coverage: the project's own AGENTS.md fact, then the global config.
    """
    for val in (
        project_facts(os.path.join(root or ".", "AGENTS.md")).get("graph_min_speedup"),
        get(cfg if cfg is not None else load(), "graph.min_speedup"),
    ):
        if val is not None:
            try:
                return float(val)
