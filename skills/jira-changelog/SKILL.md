...
4. `partial: true` in the meta is a command, not a failure to fix from `hint`: run the meta's `resume` line once, exactly as printed. Every issue that finished is cached, so the rerun asks Jira only for what is missing. Still `partial: true` → `friction-log` type `missing-info` quoting `reason`, `rows_written`, `issues_complete` and `issues_incomplete_count`. STOP. What each `reason` means and which of them a rerun cannot fix: `references/jira-changelog.md` §Partial results.
