...
from agentdata.fleet import events as E, probe as PR, registry, serve as S, skins
...
def test_the_ink_payload_is_inside_its_budget_and_three_is_not_in_it():
    ...
    # Every one but the example is a skin skins.py offers (#249-#253).
    assert "example.js" in SKINS and all(n[:-3] in skins.SKINS for n in SKINS if n != "example.js"), SKINS
...
def test_the_server_writes_the_probe_class_on_the_desk_and_nowhere_else(fleet_home):
    ...
    try:
        ...
        # paper skins, which it does (#249-#253).
        shipped = S.ink_skins()
        assert f'data-ink-skins="{" ".join(shipped)}">' in body_of("?x=1")
        assert "example" in shipped and all(n in skins.SKINS for n in shipped if n != "example"), shipped
