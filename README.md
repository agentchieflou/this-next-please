...
| `docs/skin-notebook.md` | the notebook and the night notebook (#249, #250): the first skin drawn with ink — the state grammar's reference marks, a ruled paper, gel inks by night, and the same table plain where WebGL is off |
...
| `docs/desk-types.md` | the desk's types without a build: `tsc --noEmit` over `common.js`, `picker.js` and `app.js` with their `<file>.d.ts` declarations (#523), what is typed and why the whole of each file is read, and what the check found (#236) |
